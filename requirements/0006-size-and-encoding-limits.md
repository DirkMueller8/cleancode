...
## Summary
As an operator, I want oversized inputs rejected, so that a single request cannot exhaust resources
or smuggle in unbounded data.

## Requirement
If a request exceeds 1,000,000 characters, or any individual field value exceeds 10,000 characters,
then the Logger shall reject it.
...
## Design notes
Boundary-value tests are the point here. Constants in one place (`Limits`). Applied at the input
boundary before schema validation. Encoding validity (valid UTF-8/JSON) can be asserted by the
service layer; the core assumes decoded text.

## Security & traceability
- **Why / rationale:** Bounds at the boundary prevent resource exhaustion and constrain tainted input
  (DSS §5 states the 1M / 10k limits).
- **Source:** DSS §5
