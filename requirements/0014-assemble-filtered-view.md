...
## Acceptance criteria
- [ ] Every field in the output is the result of its disposition's filter.
- [ ] Field order and names are preserved.
- [ ] Given the schema above, the raw example produces exactly the filtered example.
- [ ] The assembler depends only on `IFieldFilter` / the registry — it contains no per-disposition logic.
