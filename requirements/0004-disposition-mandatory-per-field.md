...
`IFilterRegistry` with a single `IsRegistered(name)` query (ISP); `Schema`'s constructor takes it and
rejects any field whose disposition name isn't registered. `Disposition` is a value object carrying a
name with **no special-casing** — `nonsensitive`/`private` are validated through the registry like any
other name, consistent with REQ-0013. Tests use a `FakeFilterRegistry` stub, so no real filters are
needed yet. REQ-0013 will grow the interface with registration + filter resolution.
