...
## Acceptance criteria
- [ ] A LogType with a timestamp and ≥1 other field is accepted.
- [ ] A LogType with only a timestamp is rejected.
- [ ] A LogType with no timestamp is rejected.
- [ ] The rejection identifies the offending LogType by name.

## Design notes
Enforced in the same schema-building validation as REQ-0002; a distinct rule so it maps to its own tests. "Identifying field" = any declared field other than the timestamp (kept simple for the core).
